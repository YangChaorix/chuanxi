...
class Form{
    ...
    public function editor($info,$value){
        ...
        if($info['setup']['edittype']=='UEditor'){
           ...
           $str .='<script>layui.use("layedit", function () {var '.$info['class'].' = layui.layedit.build("'.$info['class'].'");})</script>';
        }
        return $str;
    }
    ...
    public function image($info,$value){
        ...
        $thumbstr ='<div class="layui-input-4"><div class="site-demo-upload">';
        $thumbstr .='<img id="'.$info['class'].'Img" src="'.$value.'">';
        $thumbstr .='<div class="site-demo-upbar"><input type="file" name="'.$field.'" lay-type="images" class="'.$info['class'].' layui-upload-file" id="pic"></div></div></div><input type="hidden"  name="'.$field.'" class="'.$field.'val" value="'.$this->data[$field].'">';
        $thumbstr.="<script> 
                        layui.use('upload', function () {
                            ...
                            upload({
                                ...
                                success: function(res, input){
                                    ".$info['class']."Img.src = '__PUBLIC__'+res.url;
    ...
    public function file($info,$value){
        ...
        $thumbstr ='<div class="layui-input-4"><div class="site-demo-upload">';
        $thumbstr .='<img id="'.$info['class'].'File" style="width:64px;" src="'.$value.'">';
        $thumbstr .='<div class="site-demo-upbar"><input type="file" name="'.$field.'" lay-type="file" class="'.$info['class'].' layui-upload-file" id="file"></div></div></div><input type="hidden"  name="'.$field.'" class="'.$field.'fval" value="'.$this->data[$field].'">';
        $thumbstr.="<script> 
                        layui.use('upload', function () {
                            var upload = layui.upload;
                            upload({
                                ...
                                ext: '".$info['setup']['upload_allowext']."', 
                                success: function(res, input){
                                    ".$info['class']."File.src = '__ADMIN__/zyupload/skins/images/fileType/'+res.ext+'.png';
