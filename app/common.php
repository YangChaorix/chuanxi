...
/**
 * filed字段返回固定数组
 * @param  array $options需要处理的数据
 * @return array|bool
 * @throws Exception
 * @throws phpmailerException
 */
function field2array($options){

    if (!is_array($options)) {
        return false;
    }
    foreach ($options as $key => $option) {
        $str=string2array($option['setup'])['options'];
        $array = preg_split('/[,;\r\n]+/', trim($str, ",;\r\n"));
        $arr=[];
...
/**
 * pics字段返回固定数组
 * @param  string $str需要处理的数据
 * @return array|bool
 * @throws Exception
 * @throws phpmailerException
 */

function pics2array($str){
    $arr=explode("||",$str);
    if (!is_array($arr)) {
        return false;
    }
...
function isLogin(){
    return session('user.user_id')?session('user.user_id'):false;
}

///档次，面积，多少室

function cPrice($Level='1',$Area='10',$Room='3'){

    $arr=[
        'Room'=>['0'=>'0室','1'=>'1室','2'=>'2室','3'=>'3室','4'=>'4室','5'=>'5室','6'=>'6室','7'=>'7室','8'=>'8室','9'=>'9室','10'=>'10室'],
        'Level'=>['1'=>'经济型','2'=>'中档型','3'=>'高档型'],
    ];
    ...
    switch ($Level) {
    case 1:
        $price = round($Area * 850) + $Room * 358;
        break;
    case 2:
        $price = round($Area * 1250) + $Room * 568;
        break;
    case 3:
        $price = round($Area * 1550) + $Room * 688;
    }



    $data['info']=$arr['Level'][$Level].' '.$arr['Room'][$Room].' '.$Area.'㎡';
    $data['price']=$price.'元';
